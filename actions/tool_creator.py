...
def tool_creator(parameters: dict, player=None, speak=None) -> str:
    ...
    try:
        ...
        # 5. Forzar la reconexión de MATT para que cargue la nueva herramienta
        if player and hasattr(player, "on_config_saved"):
            from threading import Timer
            def delayed_reload():
                try:
                    # Esto forzará que MATTLive corte la sesión, reconstruya _build_config (con la nueva herramienta) y reconecte.
