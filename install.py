...
def main():
    print_banner()
    print("Este asistente preparará a MATT para funcionar de forma óptima.")
    print()
    ...
    archivos_basura = ["MATT.log", "MATT_Beta_Installer.exe"]
    for f in os.listdir("."):
    ...
    os.system("cls")
    print_banner()
    print("\033[36m [FASE 3/5] - Instalando dependencias de MATT...\033[0m")
    print()
    ...
    print("\033[33m[REQUERIDO] Por favor ingresa los datos fundamentales de MATT:\033[0m")
    print()
    
    # 1. Name
    name_prompt = f" ¿Cómo quieres que te llame MATT? [{existing_name}]: " if existing_name else " ¿Cómo quieres que te llame MATT?: "
    ...
    while not user_name:
        ...
        user_name = input(" ¿Cómo quieres que te llame MATT?: ").strip()
    ...
    try:
        ...
        target_vbs = os.path.join(current_dir, "Iniciar MATT Beta.vbs")
        
        # Crear acceso directo con PowerShell
        ps_cmd = (
            f"$s=(New-Object -ComObject WScript.Shell).CreateShortcut(([System.Environment]::GetFolderPath('Desktop')+'\\MATT AI.lnk'));"
            f"$s.TargetPath='{target_vbs}';"
            f"$s.WorkingDirectory='{current_dir}';"
            f"$s.IconLocation='{icon_path}';"
            f"$s.Description='Lanzador de MATT AI (Admin)';"
            f"$s.Save()"
        )
        ...
        try:
            ...
            lnk_path = os.path.join(desktop, "MATT AI.lnk")
        ...
        print("\033[32m[OK] Acceso directo 'MATT AI' creado en el Escritorio (con permisos de Admin).\033[0m")
    except Exception as e:
    ...
    print("=======================================================================\033[0m")
    print()
    print("MATT está listo para servirte.")
    ...
    print()
    print(" [1] Iniciar MATT ahora mismo")
    print(" [2] Salir")
    print()
    ...
    if launch_opt == "1":
        print("Iniciando MATT...")
        try:
            # Ejecutar el VBS silencioso
            os.startfile("Iniciar MATT Beta.vbs")
        except Exception:
            # Fallback si no está asociado
            subprocess.Popen(["wscript.exe", "Iniciar MATT Beta.vbs"])
            
    print("\nGracias por usar el instalador de MATT AI.")
    time.sleep(2)
